...
app.listen(8080, '209.97.186.161', function(){
...
io.on('connection', function (socket) {
        ...
        socket.on('createdNewMeal', function () {
                io.sockets.to('usertype_manager').emit('meal_created');
        });

        socket.on('mealTerminated', function () {
                io.sockets.to('usertype_manager').emit('meal_terminated');
                io.sockets.to('usertype_cook').emit('meal_terminated');
        });

        socket.on('inform-managers-waiters-new-table', function (user, tableNumber) {
                io.sockets.to('usertype_manager').emit('refresh_get_table_numbers');
                io.sockets.to('usertype_waiter').emit('refresh_get_table_numbers');
        });

         socket.on('inform-item-alteration', function (user, itemId) {
                io.sockets.emit('refresh_items');
        });
});
